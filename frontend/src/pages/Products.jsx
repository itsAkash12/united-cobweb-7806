...
function Products() {
 ...
 <ProductBanner/>
   ...
   <div className='mt-8 w-fit ' style={{ 
...
<div className=' border-2 p-3' key={el.id}>
    ...
    <p className={`text-2xl font-bold  text-sky-500 cursor-pointer hover:underline my-2 ${el.style}`} >{el.title}</p>
  
   <p className='text-sm text-gray-400 '>Get all {el.title}</p>
   <Link to={el.link}> <button className='bg-blue-900 text-white rounded-md py-1 mt-6 text-sm px-4 w-fit'>Shop Now</button></Link>
  </div>
    ))
  }
